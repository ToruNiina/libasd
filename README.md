...
A C++ header-only library to read an `.asd` file generated by high-speed AFM.

libasd (as a C++ library) depends only on the C++ standard library.

libasd has a wrapper library for Python3 depending on
...
(to use it as a C++ library, it does not required).

### Building Python Library

Using CMake. CMake will automatically download pybind11 as a git submodule,
so it can be built just by running following scripts.
...
by using `Buffer protocol` relying on [pybind11](https://github.com/pybind/pybind11).
...
random accessible. But `ContiguousContainer` concept is not required (e.g.
`std::deque` can be used as a backend container).
